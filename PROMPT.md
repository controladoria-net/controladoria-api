Faça um plano de ação detalhado para criação do endpoint de busca de processo por id e todo processo que for buscado será adicionado em uma tabela do banco de dados da aplicação

- endpoint: /processos/consultar/:id
- O endpoint é privado
- Use o GeneralResponseDTO para todos os retornos desse enpoint, inclusive de erros
- Deixe somente 1 try catch no router, use o padrão either para o tratamento dos demais - erros
- Siga a arquitetura da aplicação
- Não assuma nada,qualquer dúvida pergunte antes da criação do plano
- Faca um cron job que irá rodar a cada 3 dias e 00:00 para percorrer a base e atualizar os os processos
- Utilize o caso de uso já existe FindLegalCaseUseCase

_Atenção_, futuramente serão implementados endpoints para classifição, identificação e elegibilidade de uma solicitação de processos usando IA

** Pode modelar a base, já esperando essas informações **

**O fluxo é o seguinte**

- documentos serão enviados para o endpoint /classificador/processar

\*\* Segue abaixo os itens que o frontend tem mapeado para a UI
xport type DocumentType =
